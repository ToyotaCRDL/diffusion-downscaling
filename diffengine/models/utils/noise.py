...
class OffsetNoise(nn.Module):
    ...
    def forward(self, latents: torch.Tensor) -> torch.Tensor:
        ...
        return noise + self.offset_weight * torch.randn(latents.shape[0], latents.shape[1], 1, 1, device=noise.device)


@MODELS.register_module()
class OffsetNoise2d(nn.Module):
    """Offset noise module for 2d data.
    Args:
    ----
        offset_weight (float): Noise offset weight. Defaults to 0.05.
    """
    ...
    def forward(self, latents: torch.Tensor) -> torch.Tensor:
        ...
        """
        noise = torch.randn_like(latents)
        return noise + self.offset_weight * torch.randn(latents.shape[0], 1, device=noise.device)
...
class PyramidNoise(nn.Module):
    ...
    def __init__(self, discount: float = 0.9, *, random_multiplier: bool = True) -> None:
        super().__init__()
    ...
    def forward(self, latents: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(16):
            ...
            noise += u(torch.randn(b, c, w, h).to(latents)) * self.discount**i
            if w == 1 or h == 1:
                break
